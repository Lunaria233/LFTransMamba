...
PROTOCALS_4X = {
    'LFSR.ALL4.EXTRA': {
        'train': [
            ...
            '/cephFS/video_lab/datasets/super_resolution/LFSR_NTIRE_2023/data_for_training/SR_5x5_4x_128_32_34567/DLFD/',
            '/cephFS/video_lab/datasets/super_resolution/LFSR_NTIRE_2023/data_for_training/SR_5x5_4x_128_32_34567/SLFD/',
            ...
            '/cephFS/video_lab/datasets/super_resolution/LFSR_NTIRE_2023/data_for_training/SR_5x5_4x_128_32_13579/DLFD/',
            '/cephFS/video_lab/datasets/super_resolution/LFSR_NTIRE_2023/data_for_training/SR_5x5_4x_128_32_13579/SLFD/',
            ...
            '/cephFS/video_lab/datasets/super_resolution/LFSR_NTIRE_2023/data_for_training/SR_5x5_4x_128_32_24568/DLFD/',
            '/cephFS/video_lab/datasets/super_resolution/LFSR_NTIRE_2023/data_for_training/SR_5x5_4x_128_32_24568/SLFD/',
            '/cephFS/video_lab/datasets/super_resolution/LFSR_NTIRE_2023/data_for_training/SR_5x5_4x_128_32_12589/EPFL/',
            '/cephFS/video_lab/datasets/super_resolution/LFSR_NTIRE_2023/data_for_training/SR_5x5_4x_128_32_12589/HCI_new/',
            '/cephFS/video_lab/datasets/super_resolution/LFSR_NTIRE_2023/data_for_training/SR_5x5_4x_128_32_12589/HCI_old/',
            '/cephFS/video_lab/datasets/super_resolution/LFSR_NTIRE_2023/data_for_training/SR_5x5_4x_128_32_12589/INRIA_Lytro/',
            '/cephFS/video_lab/datasets/super_resolution/LFSR_NTIRE_2023/data_for_training/SR_5x5_4x_128_32_12589/Stanford_Gantry/',
            '/cephFS/video_lab/datasets/super_resolution/LFSR_NTIRE_2023/data_for_training/SR_5x5_4x_128_32_12589/DLFD/',
            '/cephFS/video_lab/datasets/super_resolution/LFSR_NTIRE_2023/data_for_training/SR_5x5_4x_128_32_12589/SLFD/',
        ],
        'val': [
...
class LF_divide_integrate_sad(object):
  ...
  def gaussian_matrix(self, M, sigma, cx=None, cy=None):
    if cx is None:
      cx = M // 2
    if cy is None:
      cy = M // 2
    ...
    y = np.arange(0, M)
    x, y = np.meshgrid(x, y)
    gaussian = (1 / (2 * np.pi * sigma**2)) * np.exp(-((x - cx)**2 + (y - cy)**2) / (2 * sigma**2))
    return gaussian
  ...
  def LFintegrate(self, LF_divided):
    ...
    pm = torch.tensor(self.gaussian_matrix(patch_size, sigma=patch_size // 5)).to(LF_divided.device)
...
def transform_train_lfsr(lr_img, hr_img, uv_img, patch_size=16, scale=4, angular=5):
...
class LFSR():
  ...
  def load(self, model, model_path, model_source):
  ...
  def build_dataloader(self, phase, dataset):
    ...
    if phase == 'train':
      ...
      return torch.utils.data.DataLoader(
          dataset=dataset,
          batch_size=cfg.train_batchsize // cfg.dist_size,
          num_workers=16,
          sampler=sampler,
          pin_memory=True,
          drop_last=True,
  ...
  def lr_scheduler(self):
    ...
    elif cfg.train_lr_scheduler == 4:
      scheduler = torch.optim.lr_scheduler.StepLR(self.Optim, step_size=150, gamma=0.5)
    elif cfg.train_lr_scheduler == 5:
      scheduler = torch.optim.lr_scheduler.StepLR(self.Optim, step_size=30, gamma=0.5)
    elif cfg.train_lr_scheduler == 6:
      print(f'using step size value: {cfg.train_lr_step_size}')
      scheduler = torch.optim.lr_scheduler.StepLR(self.Optim, step_size=cfg.train_lr_step_size, gamma=0.5)
    print(str(scheduler))
    return scheduler
    ...
    while self.Epoch < cfg.train_epoch:
      ...
      for lr_images, hr_images in train_loader:
        ...
        if self.Step % cfg.log == 0:
          print(f'Epoch: {self.Epoch}, Step: {self.Step}, Loss: {loss.item()}')
  ...
  def inference(self, lr_images):
    ...
    if cfg.processor == 'full':
      if cfg.tta:
        return self.inference_tta(lr_images)
      else:
        return self.inference_patch(lr_images)
  ...
  def val(self, loader=None, **kwargs):
    ...
    cache = {'path': cfg.model_path, 'name': cfg.name, 'psnr': 0.0, 'ssim': 0.0, 'pred': [], 'image': []}
    ...
    for lr_images, hr_images in loader:
      ...
      mean_psnr.append(metric['psnr'])
      mean_ssim.append(metric['ssim'])
      print('{}/{}, psnr: {}, ssim: {}'.format(step, total_step, metric['psnr'], metric['ssim']))

    # stat
    mean_psnr = np.mean(mean_psnr)
    mean_ssim = np.mean(mean_ssim)
    print(f'mean_psnr: {mean_psnr}, mean_ssim: {mean_ssim}, dataset: {cfg.dataset}')
  ...
  def val_all(self):
    ...
    for dataset in ['LFSR.EPFL', 'LFSR.HCInew', 'LFSR.HCIold', 'LFSR.INRIA', 'LFSR.STFgantry']:
      ...
      mean_psnr, mean_ssim = self.val()
      total_psnr.append(mean_psnr)
      total_ssim.append(mean_ssim)
    ...
    keys = ['mean_psnr', 'mean_ssim', 'real_psnr', 'synth_psnr']
    vals = [np.mean(total_psnr), np.mean(total_ssim), np.mean(real_psnr), np.mean(synth_psnr)]
    print(f'mean_psnr: {vals[0]}, mean_ssim: {vals[1]}, real_psnr: {vals[2]}, synth_psnr: {vals[3]}')
  ...
  def test(self, **kwargs):
    ...
    # save
    pwd = os.getcwd()
    submission = f'submission-{cfg.name}.zip'
    os.system(f'cd {root} && zip -r {submission} Real Synth')
    os.system(f'cd {pwd}')
    print(f'Finished on {os.getcwd()}/{root}/{submission}')
...
if __name__ == "__main__":
  ...
  # ---------------------------------------------
  parser.add_argument('--accelerate', action='store_true', help='accelerate training')
  ...
  parser.add_argument('--master', type=bool, default=True, help="master node.")
  ...
  parser.add_argument('--train-lr-step-size', type=int, default=1, help="lr scheduler step size.")
