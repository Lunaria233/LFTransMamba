...
class LF_DET_0214(nn.Module):
  def __init__(self, angRes_in, scale_factor, use_flashatten=True, patch_size=32):
    super(LF_DET_0214, self).__init__()
    channels = 160  # 64 -> 48
    ang_num_heads = 4
    spa_num_heads = 8
    ang_mlp_ratio = 4
...
class MixTransfomerBlock(nn.Module):
  ...
  def forward(self, feature, angRes, patch_size):
    ...
    # Subsequent Spatial Transformer
    for i in range(self.spa_trans_num):
      tokens = self.forward_spatial_transform_token(tokens, i)
